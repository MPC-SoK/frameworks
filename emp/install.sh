...
#Fix issue with segfaults
sed -i '/using std::endl;/a using std::max;' ~/emp-ag2pc/emp-ag2pc/2pc.h
sed -i 's/labels = new block\[cf->num_wire\];/labels = new block\[max(total_pre,cf->num_wire)\];/' ~/emp-ag2pc/emp-ag2pc/2pc.h 
